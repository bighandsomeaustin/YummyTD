...
for r in range(1, 101):
    ...
    if r == 1:
        ...
        segments.append({"enemies": (["ANT"] * 10 + ["HORNET"] * 5 + ["DRAGONFLY"]) * 8,
                         "spawn_interval": 750,
                         "delay": 1000})
        segments.append({"enemies": ["TERMITE"],
                         "spawn_interval": 10,
                         "delay": 0})
        ...
        for i in range(5):
            segments.append({"enemies": (["HORNET"] * 10 + ["DRAGONFLY"]) * 1,
                             "spawn_interval": 500,
                             "delay": 0})

    elif r == 18:
        segments.append({"enemies": ["SPIDER"] * 35,
                         "spawn_interval": 1500,
                         "delay": 3500})
        segments.append({"enemies": ["DRAGONFLY"] * 3,
                         "spawn_interval": 15,
                         "delay": 3500})

    elif r == 19:
        segments.append({"enemies": ["DRAGONFLY"] * 15,
                         "spawn_interval": 1500,
                         "delay": 3500})
        segments.append({"enemies": ["ANT"] * 35,
                         "spawn_interval": 5,
                         "delay": 0})

    elif r == 20:
        segments.append({"enemies": ["FIREFLY"] + ["ANT"] * 35 + ["TERMITE"] * 2,
                         "spawn_interval": 10,
                         "delay": 0})
        ...
        for i in range(4):
            ...
            segments.append({"enemies": ["CENTIPEDE"] * 2,
                             "spawn_interval": 1000,
                             "delay": 2000})
            segments.append({"enemies": ["TERMITE"] * 2,
                             "spawn_interval": 1000,
                             "delay": 2000})
        ...
        segments.append({"enemies": (["FIREFLY"] + ["ANT"] * 20),
                         "spawn_interval": 5,
                         "delay": 0})
        segments.append({"enemies": (["FIREFLY_ALT1"] + ["ROACH_ALT1"] * 5 + ["ROACH_QUEEN_ALT1"] * 2),
                         "spawn_interval": 5,
                         "delay": 0})
        ...
        for i in range(5):
            segments.append({"enemies": ["HORNET"] * 10,
                             "spawn_interval": 150,
                             "delay": 1500})
            segments.append({"enemies": ["HORNET_ALT1"] * 5,
                             "spawn_interval": 500,
                             "delay": 500})
            segments.append({"enemies": ["SPIDER"] * 5,
                             "spawn_interval": 150,
                             "delay": 1500})
            segments.append({"enemies": ["SPIDER_ALT1"] * 2,
                             "spawn_interval": 500,
                             "delay": 500})

    elif r == 37:
        segments.append({"enemies": ["ROACH_ALT1"] * 10,
                         "spawn_interval": 150,
                         "delay": 500})
        segments.append({"enemies": ["DRAGONFLY_ALT1"] * 5,
                         "spawn_interval": 150,
                         "delay": 4500})
        segments.append({"enemies": ["DUNG_BEETLE"],
                         "spawn_interval": 150,
        ...
        for i in range(5):
            segments.append({"enemies": ["CENTIPEDE_ALT1"] * 2,
                             "spawn_interval": 150,
                             "delay": 500})
            segments.append({"enemies": ["CENTIPEDE_BOSS_ALT1"],
                             "spawn_interval": 150,
                             "delay": 500})
            segments.append({"enemies": ["CENTIPEDE"] * 2,
                             "spawn_interval": 150,
        ...
        segments.append({"enemies": ["FIREFLY_ALT1"] * 10,
                         "spawn_interval": 5,
                         "delay": 0})
        segments.append({"enemies": ["ROACH_QUEEN_ALT1"],
                         "spawn_interval": 5,
                         "delay": 0})
        segments.append({"enemies": ["ROACH"] * 10,
                         "spawn_interval": 150,
        ...
        segments.append({"enemies": ["DUNG_BEETLE"],
                         "spawn_interval": 150,
                         "delay": 6000})
        segments.append({"enemies": ["BEETLE_ALT1"] * 15,
                         "spawn_interval": 650,
                         "delay": 0})

    elif r == 44:
        segments.append({"enemies": ["DRAGONFLY_ALT1"] * 25,
                         "spawn_interval": 350,
                         "delay": 0})
        segments.append({"enemies": ["DRAGONFLY"] * 30,
                         "spawn_interval": 350,
                         "delay": 0})

    elif r == 45:
        segments.append({"enemies": ["MILLIPEDE"] * 2,
                         "spawn_interval": 8000,
                         "delay": 0})
    ...
    # BALANCE MORE BETWEEN HERE!!!
        ...
        segments.append({"enemies": ["MILLIPEDE_ALT1"],
                         "spawn_interval": 150,
                         "delay": 4000})
        segments.append({"enemies": ["CENTIPEDE"] * 20,
                         "spawn_interval": 1250,
                         "delay": 0})
        ...
        segments.append({"enemies": ["MILLIPEDE"] * 3,
                         ...
                         "delay": 8500})
        for i in range(10):
            segments.append({"enemies": ["ANT_ALT1"] * 30,
                             "spawn_interval": 25,
                             "delay": 1500})
...
def send_wave(scrn: pygame.Surface, round_number: int) -> bool:
    ...
    if segment_enemy_spawned < len(segment["enemies"]):
        if pygame.time.get_ticks() - segment_start_time >= effective_interval:
            ...
            elif enemy_type == "MANTIS_ALT1":
                new_enemy = (game_tools.MantisBoss(spawn_pos_alt1, offset_path_alt1))
            elif enemy_type == "TERMITE_ALT1":
                new_enemy = (game_tools.TermiteEnemy(offset_path_alt1, spawn_pos_alt1))
