...
from enemies import AntEnemy, BeetleEnemy, HornetEnemy, MantisBoss, CentipedeEnemy, DungBeetleBoss, RoachMinionEnemy, RoachQueenEnemy, FireflyEnemy, DragonflyEnemy, SpiderEnemy, TermiteEnemy, MillipedeBoss
towers = []
enemies = []
enemies_spawned = 0
wave_size = 0
            ...
            if detect_single_click():
                if tower.curr_top_upgrade == 0 and money >= 1250:
                    ...
                    if tower.curr_bottom_upgrade == 0:
                        ...
                        tower.projectile_image = "assets/rat_recruit_faster.png"
                    elif tower.curr_bottom_upgrade == 1:
            ...
            if detect_single_click():
                if money >= 375 and tower.curr_bottom_upgrade == 0:
                    ...
                    if tower.curr_top_upgrade == 0:
                        ...
                        tower.image = load_image("assets/alfredo_ozbourne_longer_riffs+amplifier.png")
                        tower.original_image = load_image("assets/alfredo_ozbourne_longer_riffs+amplifier.png")
                        tower.projectile_image = "assets/alfredo_ozbourne_longer_riffs+amplifier.png"
...
def handle_newtower(scrn: pygame.surface, tower: str) -> bool:
    ...
    if tower == "NULL":
        ...
        if within_spawn_point((mouse[0], mouse[1]), recruit_path, radius=50):
            ...
            scrn.blit(text_checkpath, (mouse[0] - 35, mouse[1] + 50))
        if detect_single_click() and check_hitbox(house_hitbox, relative_pos, tower):
            tower_rattent = RatTent((mouse[0], mouse[1]), radius=50, recruit_health=1, speed=1,
                                    damage=1,
...
class RatTent:
    def __init__(self, position, radius, recruit_health, speed, damage, image_path, projectile_image,
                 spawn_interval=3000):
    ...
    def update(self, enemies):
        ...
        if self.curr_top_upgrade < 2:
            if pygame.time.get_ticks() - self.last_shot_time >= scaled_interval and RoundFlag:
                recruit_entity = RecruitEntity(self.position, 1, 1, recruit_path, 1, self.projectile_image)
